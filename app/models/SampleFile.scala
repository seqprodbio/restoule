...
case class SampleFile(id: Option[Int], sampleLimsInfoId: Option[Int], fileName: String, path: String, origin: String, swid: Int, sample: String, library: String, sequencerRunName: String, lane: Int, barcode: String, read: Int, createdInDB: java.sql.Timestamp)
...
class SampleFileTable(tag: Tag) extends Table[SampleFile](tag, "sample_file") {
   ...
   def * = (id.?, sampleLimsInfoId.?, fileName, path, origin, swid, sample, library, sequencerRunName, lane, barcode, read, createdInDB) <> (SampleFile.tupled, SampleFile.unapply)
