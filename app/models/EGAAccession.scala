...
case class EGAAccession(id: Option[Int], resourceType: String, accession: String, refname: String, created: java.sql.Timestamp)
...
class EGAAccessionTable(tag: Tag) extends Table[EGAAccession](tag, "ega_accession") {
   ...
   def * = (id.?, resourceType, accession, refname, created) <> (EGAAccession.tupled, EGAAccession.unapply)
}